...
const debug = false;
let threadObj: Java.Wrapper<object>;
let accessoryFD: number;

if (Java.available) {
  if (debug) {
    log(
      "Java was available, attempting to create a thread and hook methods...",
    );
    Java.perform(() => {
  ...
  Java.perform(() => {
    ...
    usbManager.openAccessory.overload(
      "android.hardware.usb.UsbAccessory",
    ).implementation = (usbAccessory: NativePointerValue) => {
      if (debug) {
        log(
          `usbManager.openAccessory(android.hardware.usb.UsbAccessory) called`,
        );
        stackTraceOutsideJava();
      }
      const ret = usbManager.openAccessory.call(this, usbAccessory);
      const parcelFileDescriptor = Java.cast(
        ret,
        Java.use("android.os.ParcelFileDescriptor"),
      );

      log(
        `***** usb accessory FD : ${parcelFileDescriptor} ${parcelFileDescriptor.getFd()}`,
      );
  ...
  log(
    "Java not available - unable to set classloader, very likely everything else will fail",
  );
}
...
const libcWrite =
  Process.findModuleByName("libc.so")?.findExportByName("write");
if (libcWrite) {
...
const libcRead = Process.findModuleByName("libc.so")?.findExportByName("read");
if (libcRead) {
  Interceptor.attach(libcRead, {
    onEnter: function (args) {
